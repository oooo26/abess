...
sim_theta <- function(p, type = 1, seed, beta = NULL) {
  if(using_seed) {
    set.seed(seed)
  }
  ...
  if(type == 2) {
    # set.seed(seed)
    sigma_inv <- fastclime::fastclime.generator(n = 20, d = p, graph = "random", verbose = FALSE)
  ...
  if(type == 3) {
    ...
    for(i in 0:(sqr - 1)) {
      for(j in 1:(sqr - 1)) {
        ...
        theta[i * sqr + j + 1, i * sqr + j] <- theta[i * sqr + j, i * sqr + j + 1]
      }
    }
    for(i in 0:(sqr - 2)) {
      for(j in 1:sqr) {
        theta[i * sqr + j, i * sqr + j + sqr] <- sample(c(-0.5, 0.5), 1)
        theta[i * sqr + j + sqr, i * sqr + j] <- theta[i * sqr + j, i * sqr + j + sqr]
      }
    }
  }
  ...
  if(type == 4) {
    ...
    for(i in 0:(sqr - 2)) {
      for(j in 1:(sqr - 1)) {
        ...
        theta[i * sqr + j + sqr + 1, i * sqr + j] <- theta[i * sqr + j, i * sqr + j + sqr + 1]
      }
    }
    for(i in 0:(sqr - 2)) {
      for(j in 2:sqr) {
  ...
  if(type == 8) theta <- gen_rr_adjmat(p, degree, beta, alpha, type = "ferro")
  if(type == 9) theta <- gen_rr_adjmat(p, degree, beta, alpha, type = "glass")
  if(type == 10) theta <- gen_4nn_cyc(p, degree, beta, alpha, type = "ferro")
  if(type == 11) theta <- gen_4nn_cyc(p, degree, beta, alpha, type = "glass")
  if(type == 12) theta <- gen_4nn_cyc(p, degree, beta, alpha, type = "glass_weak")
...
#'
#' @examples
generate.bmn.freq.data <- function(n, p, type = 1, seed, beta = NULL) {
  theta <- sim_theta(p, type, seed, beta)
  ...
  seed1 <- sample(1: xx, 1)
  seed2 <- sample((1: xx)[-seed1], 1)
  
  train <- sample_by_conf(n = n, theta = theta, seed = seed1)
  valid <- sample_by_conf(n = n, theta = theta, seed = seed2)
  return(list(train = train, valid = valid, theta = theta))
}
...
#'
#' @examples
generate.bmn.data <- function(n, p, type = 1, seed, beta) {
  theta <- sim_theta(p, type, seed, beta)
  Ising_Gibbs()
}
