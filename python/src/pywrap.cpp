...
std::tuple<Eigen::VectorXd> pywrap_DAG(Eigen::MatrixXd x_Mat, int n, int p, int normalize_type, int max_iter,
                                       int exchange_num, int path_type, bool is_warm_start, int ic_type, double ic_coef,
                                       int Kfold, Eigen::VectorXi gindex_Vec, Eigen::VectorXi sequence_Vec,
                                       Eigen::VectorXi cv_fold_id_Vec, int s_min, int s_max, int screening_size,
                                       Eigen::VectorXi always_select_Vec, bool early_stop, int thread,
                                       bool sparse_matrix, int splicing_type, int sub_search,
                                       Eigen::VectorXi A_init_Vec) {
    List mylist =
        abessDAG_API(x_Mat, n, p, normalize_type, max_iter, exchange_num, path_type, is_warm_start, ic_type, ic_coef,
                     ...
                     thread, sparse_matrix, splicing_type, sub_search, cv_fold_id_Vec, A_init_Vec);

    Eigen::VectorXd beta;
    mylist.get_value_by_name("beta", beta);
    return std::make_tuple(beta);
}
